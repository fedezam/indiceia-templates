...
El template consume URLs públicas de imágenes. **No hostea ni procesa imágenes** — eso es responsabilidad del comercio.

El campo `imagen` en cada ítem de GOODS es opcional. Si no existe o la URL falla, el template muestra un emoji placeholder automáticamente.

### Cómo obtener URLs de imágenes

**Opción 1 — Desde cualquier sitio (más simple)**

Si el producto ya tiene foto publicada en Instagram, MercadoLibre, Facebook u otro sitio:
1. Abrí la foto en el navegador
2. Click derecho → **Copiar dirección de imagen**
3. Usar esa URL en el campo `imagen` del producto

**Opción 2 — ImageKit.io (recomendada para fotos propias)**

[imagekit.io](https://imagekit.io) es un servicio gratuito de hosting de imágenes con CDN mundial. Cada comercio usa su propia cuenta — las imágenes son suyas.

1. Entrá a imagekit.io → *Sign up with Google* (un clic)
2. Subí la foto del producto
3. Copiá la URL de la imagen
4. Usala en el campo `imagen` del producto

> Free tier: 20GB storage, 20GB bandwidth/mes. Sin tarjeta de crédito.

### Reglas para imágenes en templates

- El template DEBE manejar gracefully la ausencia de imagen (`imagen` ausente o URL rota)
- Usar `object-fit: cover` para que cualquier proporción se vea bien
- Mostrar un fallback (emoji, ícono, color de fondo) cuando la imagen no carga
- No asumir que las imágenes son cuadradas — pueden ser de cualquier proporción
